...
- (instancetype)initWithText:(NSString *)text {
    if ((self = [self initWithFrame:CGRectZero])) {
        ...
        _textLabel.frame = CGRectMake(10.0, 5.0, 310.0, 20.0);
...
+ (void)toastInView:(UIView *)parentView withText:(NSString *)text {
    ...
    CGSize textSize = [text boundingRectWithSize:CGSizeMake(300, 1000) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName : view.textLabel.font} context:nil].size;
    
    // Change toastview frame
    if ((pWidth - lWidth - 20) / 2. <= 0)
    {
        view.frame = CGRectMake(5.0, pHeight - lHeight - 60, 310.0, textSize.height);
        view.textLabel.frame = CGRectMake(5.0, 0.0, 300.0, textSize.height);
    }
    else
    {
